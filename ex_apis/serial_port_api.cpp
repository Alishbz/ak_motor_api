...
{
    std::string wPortName(_port_name.begin(), _port_name.end()); 

    if (m_windows_ll_connect(wPortName.c_str())) {
        m_state = true;
...
bool serial_port_api::open(std::string _port_name)
{
    ...
    if (m_state == false &&
        m_windows_ll_connect(wPortName.c_str())) {
        m_state = true;
        m_port_name = _port_name;
    }
...
bool serial_port_api::m_windows_ll_connect(const char* port_name)
{ 
